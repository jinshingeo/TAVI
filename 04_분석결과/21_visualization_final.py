...
# ── 데이터 로드 ────────────────────────────────────────────────────────
with open(os.path.join(BASE, 'catchment_corrected_summary.json'), encoding='utf-8') as f:
    d_idw = json.load(f)
...
utci_idw = pd.read_csv(os.path.join(BASE, 'link_utci_corrected.csv'), encoding='utf-8-sig')
utci_sol = pd.read_csv(os.path.join(BASE, 'link_utci_solweig.csv'),   encoding='utf-8-sig')
